...
namespace Yit.Cache.Cache.Memory
{
    public class MemoryCacheImp : ICache
    {
        ...
        public bool SetCache(string key, string value, DateTime? expireTime = null)
        {
            try
            {
                if (expireTime == null)
        ...
        public bool SetCacheSliding<T>(string key, T value, TimeSpan? expireTime = null)
        {
            try
            {
                ...
                options.SlidingExpiration = expireTime;
                return cache.Set(key, value, options) != null;
            }
            catch (Exception ex)
            {
                LogHelper.Error(ex);
        ...
        {
            var hashFields = cache.Get<Dictionary<string, string>>(key);
            foreach (KeyValuePair<string, string> keyValuePair in hashFields.Where(p => dict.Keys.Contains(p.Key)))
            {
                dict[keyValuePair.Key] = keyValuePair.Value;
            }
            return dict;
